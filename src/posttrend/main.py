...
def train():
    ...
    try:
        Posttrend().crew().train(n_iterations=int(sys.argv[1]), filename=sys.argv[2], inputs=inputs)
...
def replay():
    ...
    """
    try:
        Posttrend().crew().replay(task_id=sys.argv[1])
...
def test():
    ...
    try:
        Posttrend().crew().test(n_iterations=int(sys.argv[1]), eval_llm=sys.argv[2], inputs=inputs)
